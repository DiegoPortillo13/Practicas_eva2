...
public class Pintar {   
  ...
  public static void pintarLinea(Graphics g, int x1,int y1,int x2,int y2,int tam){
        int xAux = 0; int yAux = 0; //Estas Nos serviran para depues escribir (dibujar) la letra de sobre la lineq
        //((Graphics2D)g).setRenderingHint(RenderingHints.KEY_ANTIALIASING,  RenderingHints.VALUE_ANTIALIAS_ON);
        BasicStroke stroke = new BasicStroke(3);
        ((Graphics2D)g).setColor(Color.RED);
        ((Graphics2D)g).setStroke(stroke);         
       ((Graphics2D)g).drawLine(x1+10, y1+10, x2+10, y2+10);//x1.y1 son de donde empieza la linea a dibujarse hacia el otro nodo.
       
       //x1 y x2 son los idetficadores de las cordenadas de los nodos a conectar
       if(x1<=x2)//Nodo 1 y nodo 2
           xAux=((x2-x1)/2)+x1;       
        if(x1>x2)
           xAux=((x1-x2)/2)+x2;
        ...
        Font fuente=new Font("Monospaced",Font.PLAIN, 16);
        g.setFont(fuente);
